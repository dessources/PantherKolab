...
const MESSAGES_TABLE =
  process.env.DYNAMODB_MESSAGES_TABLE || "PantherKolab-Messages-dev";
...
async function generateSummary(
  ...
  console.log(
    `[Summary] Generating summary for conversation ${conversationId}`
  );
...
Format your response as clear, actionable bullet points. Use a friendly, student-oriented tone. As the user won't have the ability to respond to your answer do not offer to clarify further nor indicate that you are ready for another question. Your answer should stop at the end of the summary.
...
async function cacheSummary(
  ...
  try {
    ...
    console.log(
      `[Summary] Cached summary ${summaryId} with TTL ${new Date(
        ttl * 1000
      ).toISOString()}`
    );
  } catch (error) {
