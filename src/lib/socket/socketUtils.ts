 ...
 */
export function getCallSession(sessionId: string): CallSessionState | undefined {
 ...
 */
export function removeParticipantFromCall(sessionId: string, userId: string): boolean {
  const session = activeCalls.get(sessionId)
  if (!session) {
    return false
  }
 ...
 */
export function emitToUser(io: Server, userId: string, event: string, data: any): boolean {
